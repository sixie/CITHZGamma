...
////invariant mass of two obejcts given, pid each. 
void calcPairEnEtaPhi(float en[],float eta[],float phi[],int pid1,int pid2,float res[]){
  ...
  for( int j= 0; j<2; j++){
     if( en[j]<mass[j]){
      cout<<"wrong e<m? " << en[j]<<" "<< mass[j]<<endl; 
      exit(1);
    }
    float p = sqrt(en[j]*en[j] - mass[j]*mass[j]); 
    float px = p /cosh(eta[j]) * cos(phi[j]);
...
////invariant mass of two obejcts given, pid each. 
void calcMass3ObjectsEn(float en[3],float eta[3],float phi[3],int pid1,int pid2,int pid3,float res[]){
