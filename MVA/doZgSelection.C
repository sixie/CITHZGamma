...
#endif
#include "modelConstants_HZG_8TeV.h"
...
#include "HZgammaMVA.cc"


void doZgSelection(string sample= "s12-zllm50-v7a", int eleOrmu =1){
  ...
  if( sample == "del"){
    fChain->Add("/afs/cern.ch/work/y/yangyong/CMSSW/CMSSW_5_2_4/src/CITHZGamma/MVA/Trees/AllNtuple_*del*root");
  }
  else if( sample == "dmu"){
    fChain->Add("/afs/cern.ch/work/y/yangyong/CMSSW/CMSSW_5_2_4/src/CITHZGamma/MVA/Trees/AllNtuple_*dmu*root");
  }else {
    isdata = 0; 
    TString file = TString(Form("/afs/cern.ch/work/y/yangyong/CMSSW/CMSSW_5_2_4/src/CITHZGamma/MVA/Trees/AllNtuple_%s_noskim_0*root",sample.c_str()));
    fChain->Add(file);
  }
  ...
  //cross section;
  map<string,double> map_cross_mc;
  
  double xs = 0; 

  if( isdata==0){

    if(sample.find("zllm50") != string::npos){
      xs =  1177.6*3;
    }else if( sample.find("zgllg") != string::npos){
      xs = 132.6*1.20; 
    }
    else if( sample.find("s12-h") != string::npos){
      string mss = sample.substr(5,3);
      int m = atoi(mss.c_str());
      int ind = int( (m-110)/0.5+0.01);
      double brhzg = smbr[ind];
      if( sample.find("gf") != string::npos){
	xs = gghxsec[ind];
      }else if( sample.find("vbf") != string::npos){
	xs = vbfxsec[ind];
      }else if( sample.find("vh") != string::npos){
	xs = whxsec[ind] +  zhxsec[ind] ;
      }else if( sample.find("tt") != string::npos){
	xs = tthxsec[ind];
      }else{
	cout<<"higgsMC ? " << sample.c_str()<<endl; 
	return; 
      }
      double brzll = 0.1099;
    
      cout<<"m/xs/br " << m<<" "<< xs <<" "<< brhzg <<endl; 
    
      xs *= brhzg * brzll; 
    
    }else{
      cout<<"sample?? " << sample.c_str()<<endl; 
      return; 
    }
  ...
  if(sample.find("s12") != string::npos){
    ...
    TH1D*hDEvents = (TH1D*)fmc->Get("hDEvents");
    ngenerated = hDEvents->GetBinContent(1);
    
    cout<<"got pileup hist " << hhdata->Integral()<<" "<< hhmc->Integral() <<" ngen "<<ngenerated<<endl; 
  ...
  filename = TString(Form("doZgSelection_%s_elemu%d.root",sample.c_str(),eleOrmu));
  ...
  mytree->Branch("hzgmva",&hzgmva,"hzgmva/F");
  
  
  mytree->Branch("photonmatchmc",&photonmatchmc,"photonmatchmc/F");
